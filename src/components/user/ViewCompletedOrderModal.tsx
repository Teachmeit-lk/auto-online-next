...
> = ({ isOpen, onClose, order }) => {
  ...
  const fetchOrderDetails = async (poId: string) => {
    ...
    try {
      ...
      const poData = await FirestoreService.getById(
      ...
      if (poData?.buyerId) {
        try {
          const buyerData = await FirestoreService.getById(
            COLLECTIONS.USERS,
            poData.buyerId
          );
  ...
  const fetchVendorOrderDetails = async (quotationId: string) => {
    ...
    try {
      ...
      const transformedData = {
        ...
        description:
          quotationData?.products?.[0]?.description ||
          quotationData?.notes ||
          quotationData?.terms,
  ...
  const productsList =
    orderData?.products || orderData?.items || orderData?.orderItems || [];
            ...
            <div className="bg-[#F8F8F8] rounded-[8px] sm:p-8 p-4 space-y-6">
              ...
              <form className="sm:grid sm:grid-cols-3 gap-y-4 gap-x-6 sm:space-y-0 space-y-2 sm:h-full h-[500px] overflow-y-auto">
                <div>
                  ...
                  <input
                    type="text"
                    value={useRefactoredIdLast("ON", orderNo)}
                    readOnly
