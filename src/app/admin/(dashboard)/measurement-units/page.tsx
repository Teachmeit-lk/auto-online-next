...
const AdminMeasurementUnitsPage: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const list = await FirestoreService.getAll<UnitDoc>(
  ...
  const onSubmit = async (data: CreateUnitForm) => {
    ...
    setError(null);
    try {
      await FirestoreService.create<UnitDoc>(COLLECTIONS.MEASUREMENT_UNITS, {
        name: data.name,
        isActive: true,
      } as any);
  ...
  const onEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    try {
      await FirestoreService.update<UnitDoc>(
        COLLECTIONS.MEASUREMENT_UNITS,
        selected.id,
