...
type FuelTypeDoc = BaseDocument & { name: string };
...
const AdminFuelTypesPage: React.FC = () => {
  const [submitting, setSubmitting] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [items, setItems] = useState<FuelTypeDoc[]>([]);
  const [loading, setLoading] = useState(true);
  const [open, setOpen] = useState(false);
  const [editOpen, setEditOpen] = useState(false);
  const [selected, setSelected] = useState<FuelTypeDoc | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const list = await FirestoreService.getAll<FuelTypeDoc>(COLLECTIONS.FUEL_TYPES, undefined, "name", "asc");
      setItems(list);
    } catch (e: any) {
      setError(e?.message || "Failed to load fuel types");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (data: CreateTypeForm) => {
    ...
    setError(null);
    try {
      await FirestoreService.create<FuelTypeDoc>(COLLECTIONS.FUEL_TYPES, { name: data.name, isActive: true } as any);
      setMessage("Fuel type created successfully.");
      reset({ name: "" });
      setOpen(false);
      await load();
    } catch (e: any) {
      setError(e?.message || "Failed to create fuel type.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const onEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    try {
      await FirestoreService.update<FuelTypeDoc>(COLLECTIONS.FUEL_TYPES, selected.id, { name: selected.name } as any);
      setMessage("Fuel type updated successfully.");
      setEditOpen(false);
      await load();
    } catch (e: any) {
      setError(e?.message || "Failed to update fuel type.");
    } finally {
      setSubmitting(false);
    }
  };

  const toggleActive = async (item: FuelTypeDoc) => {
    if (!item?.id) return;
    setSubmitting(true);
    setError(null);
    try {
      await FirestoreService.update<FuelTypeDoc>(COLLECTIONS.FUEL_TYPES, item.id, { isActive: !item.isActive } as any);
      await load();
    } catch (e: any) {
    ...
    <div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td className="px-4 py-3" colSpan={3}>No fuel types found.</td></tr>
              ) : (
                items.map((it) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded shadow-lg p-5 w-full max-w-lg">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold">Create Fuel Type</h2>
      ...
      {editOpen && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded shadow-lg p-5 w-full max-w-lg">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold">Edit Fuel Type</h2>
            ...
            <form onSubmit={onEditSubmit} className="space-y-4">
              <div>
                ...
                <input
                  value={selected.name || ""}
                  onChange={(e) => setSelected({ ...(selected as FuelTypeDoc), name: e.target.value })}
...
export default AdminFuelTypesPage;
