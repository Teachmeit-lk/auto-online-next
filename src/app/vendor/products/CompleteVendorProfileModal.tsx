...
interface ICompleteVendorProfileModalProps {
  isOpen: boolean;
  onConfirm: () => void;
  onClose: () => void;
}

export const CompleteVendorProfileModal: React.FC<
  ICompleteVendorProfileModalProps
> = ({ isOpen, onConfirm, onClose }) => {
  return (
    <Dialog.Root open={isOpen} onOpenChange={onClose}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/40 backdrop-blur-none" />
        <Dialog.Content className="fixed top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] h-[180px] bg-white rounded-[8px] shadow-lg focus:outline-none">
          ...
          <div className="bg-[#F8F8F8] rounded-[5px] ml-4 mr-4 mt-2 p-4 flex flex-col items-center justify-center">
            <p className="text-[11px] text-[#111102] font-body font-[500] text-center">
              Please complete your vendor profile details to add products.
            </p>

            <div className="flex justify-center gap-x-6 mt-8">
              <button
                onClick={onConfirm}
                className="w-[120px] h-[24px] bg-[#F9C301] text-[#111102] font-[600] font-body text-[11px] rounded-[4px] hover:bg-yellow-500"
              >
                Go to Profile
              </button>
