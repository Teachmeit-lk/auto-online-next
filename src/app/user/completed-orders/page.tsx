...
import { FirestoreService, COLLECTIONS, Order, PurchaseOrder } from "@/service/firestoreService";
...
const CompletedOrders: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setLoading(true);
      try {
        const list = await FirestoreService.getAll<Order>(
          COLLECTIONS.ORDERS,
        ...
        const sorted = [...list].sort((a: any, b: any) => (toMs(b?.completedDate || b?.createdAt) - toMs(a?.completedDate || a?.createdAt)));
        setOrders(sorted);
        ...
        const deliveredSorted = [...poList].sort((a: any, b: any) => (toMs(b?.updatedAt || b?.createdAt) - toMs(a?.updatedAt || a?.createdAt)));
  ...
  const rows = useMemo(() => {
    ...
    const sorted = combined.sort((a: any, b: any) => new Date(b.dateCompleted).getTime() - new Date(a.dateCompleted).getTime());
  ...
  const filtered = useMemo(() => {
    ...
    return rows
      .filter((r) =>
        ...
        String(r.totalAmount ?? "").toLowerCase().includes(q)
      )
      .slice(0, entries);
  }, [rows, search, entries]);
  ...
  return (
    <TabLayout type="user">
      <div className="w-full p-8 bg-[#F8F8F8] rounded-tr-[15px] rounded-br-[15px] rounded-bl-[15px] " id="quotationrequests">
        <h1 className="text-[18px] font-bold font-body text-center text-[#111102] mb-6">Completed Orders</h1>

        <div className="flex flex-row items-center justify-between  mb-4">
          <div>
            <div className="font-body font-[500] text-[14px]  text-[#111102] mb-1">Show</div>
          ...
          <div>
            <div className="font-body font-[500] text-[14px]  text-[#111102] mb-1">Search</div>
            <div className="relative flex items-center rounded-[5px] text-sm text-gray-600 w-[263px] h-[28px]">
              ...
              <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
                <Search strokeWidth="2px" color="#5B5B5B" size="17px" className="text-gray-600" />
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="w-full border-collapse ">
            <thead>
              <tr className="h-[36px] bg-[#D1D1D1] text-center text-[14px] font-body text-[#111102] font-[500] ">
                <th className="border border-r-2 border-b-2 border-white px-[5px]  py-2 ">No.</th>
                <th className="border border-r-2 border-b-2  border-white  py-2">Order No</th>
                <th className="border border-r-2 border-b-2 border-white py-2">Vendor Code</th>
                <th className="border border-r-2 border-b-2 border-white  py-2">Company Name</th>
                <th className="border border-r-2 border-b-2 border-white py-2">Total Amount</th>
                <th className="border border-r-2 border-b-2 border-white py-2">Completed Date</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td className="px-4 py-3" colSpan={7}>No completed orders.</td></tr>
              ) : filtered.map((row, index) => (
                <tr key={index} className="hover:bg-gray-50 bg-white text-[12px] font-body text-[#111102] ">
                  <td className="border border-r-2 border-b-2  border-[#F8F8F8]  py-2 text-center">{row.no}</td>
                  <td className="border border-r-2 border-b-2 border-[#F8F8F8] pl-7 py-2 ">{row.orderNo}</td>
                  <td className="border border-r-2 border-b-2 border-[#F8F8F8] pl-7 py-2 ">{row.vendorCode}</td>
                  <td className="border border-r-2 border-b-2 border-[#F8F8F8] pl-7 py-2 ">{row.vendorName}</td>
                  <td className="border border-r-2 border-b-2 border-[#F8F8F8] pl-7 py-2 ">{row.totalAmount}</td>
                  <td className="border border-r-2 border-b-2 border-[#F8F8F8] pl-7 py-2 ">{row.dateCompleted}</td>
        ...
        <div className="mt-4 text-[12px] text-[#5B5B5B] font-body">Showing 1-{Math.min(entries, rows.length)} Entries</div>

        <ViewCompletedOrderModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} order={selected as any} />
      </div>
    </TabLayout>
  );
};
